...
void setup() {
  ...
  for(int i=0;i<100;i++) executedProjects[i] = false;
  ...
  for(int k=0; k<granularity; k++){
    for(int l=0; l<granularity; l++){
      for(int i=0; i<projects; i++){
        if(instructions[i][0]*granularity/maxX<k && instructions[i][1]*granularity/maxX<l && executedProjects[i]== false){
          Serial.println("---");
          Serial.print(instructions[i][1]);Serial.print(", "); Serial.println(instructions[i][0]);
          Serial.print(i); Serial.print(", k&x "); Serial.print(k); Serial.print(", "); Serial.print(instructions[i][0]*granularity/maxX); Serial.print(", l&y "); Serial.print(l); Serial.print(", "); Serial.print(instructions[i][1]*granularity/maxX); ; Serial.print(", val: "); Serial.println(instructions[i][4]);
          build(instructions[i][1],instructions[i][0],instructions[i][3],instructions[i][2], instructions[i][4]); // Don't forget that x and y axises are mixed up  
          executedProjects[i]=true;
        }
...
void build(int x, int y, int sizeX, int sizeY, int val){
  
  if(val >= 7){
    drawRoad(x,y,sizeX,sizeY);
  }
  
  else if(val == 5){
    drawPark(x,y,sizeX,sizeY);
  }
...
void drawRoad(int x, int y, int sizeX, int sizeY){
  int splits = 4;
  int narrow = min(sizeX,sizeY);
  int change = narrow/splits;
  int nudge = 0;
  if(narrow == sizeX){
    for(int i = 0; i < splits; i++){
      draw(x-nudge,y,change,sizeY);
      nudge += change;
    }
  }
  if(narrow == sizeY){
    for(int i = 0; i < splits; i++){
      draw(x,y-nudge,sizeX,change);
      nudge += change;
    }
  }
...
void drawPark(int x, int y, int sizeX, int sizeY){
  ...
  draw(x,y,sizeX,sizeY);
  moveTo(x-sizeX/2, y+sizeY/2);
  
  for(int i=0; i < 50 ;i++){
    int wiggleY = random(-20,20);
    int wiggleX = random(-20,20);
    
    wiggle(wiggleX, wiggleY);
    
    if(wiggleX + corePosX > wMaxX || wiggleX + corePosX < wMinX ||  wiggleY + corePosY > wMaxY || wiggleY + corePosY < wMinY){
      moveTo(x-sizeX/2, y+sizeY/2);  
    }
  }
